...
import { WHATSAPP_TRABAJA } from "@/lib/siteConfig";

export function TrabajaConNosotrosCTA() {
  const message = encodeURIComponent(
    "Hola, me gustaría trabajar con ustedes. ¿Tienen vacantes o información sobre oportunidades laborales?"
  );
  const link = `https://wa.me/${WHATSAPP_TRABAJA}?text=${message}`;

  return (
    <section className="relative py-16 sm:py-20 md:py-28 bg-yellow-400 flex flex-col items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-8 sm:p-12 flex flex-col items-center">
        <h2 className="text-3xl sm:text-4xl font-bold text-blue-900 mb-4">Trabaja con Nosotros</h2>
        <p className="text-gray-700 text-lg mb-6 text-center max-w-xl">
          ¿Te gustaría formar parte de nuestro equipo? Haz clic en el botón para contactarnos por WhatsApp y conocer oportunidades laborales.
        </p>
        <a
          href={link}
          target="_blank"
          rel="noopener noreferrer"
          className="bg-[#25D366] text-white px-8 py-4 rounded-full font-semibold text-lg flex items-center gap-2 shadow-lg hover:scale-105 transition"
        >
